...
* написать через cgo биндинги к golang для любого сишного кода/библиотеки
* написать тесты на го к этому коду
* убедиться что нет утечек памяти - погонять через bench
* закоммить код, необходимый для компилции - полный код это это какая-то редкая либа, инструкции по установке если это публичная либа
